# """Phase 1 -- Predict Yield: enter raw weather + soil readings, get predicted harvest yield."""

# import streamlit as st

# from streamlit_app.api_client import ApiError, predict
# from streamlit_app.components.forms import render_full_input_form
# from streamlit_app.components.utils import render_prediction_metrics, sidebar_run_model_selector
# from streamlit_app.theme import hero, inject_theme

# st.set_page_config(page_title="Predict Yield", page_icon="🌾", layout="wide")
# inject_theme()
# hero("🌾 Phase 1 — Predict Yield", "Enter raw weekly weather + soil readings to predict final harvest yield.")
...
#     This predicts the final, end-of-season harvest yield from the raw inputs below.
#     </div>
#     """,
#     unsafe_allow_html=True,
# )

# raw_row = render_full_input_form(schema, key_prefix="predict")

# st.markdown("---")
# if st.button("🌱 Predict Yield", type="primary", use_container_width=True):
#     with st.spinner("Running raw inputs through the model pipeline..."):
#         try:
#             result = predict(run_tag, model_name, raw_row)
